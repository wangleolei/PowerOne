...
<link href="/taobaoke/Public/Home/images/favicon.ico" rel="icon" type="image/x-icon" />
	...
	<link href="/taobaoke/Public/Home/css/article.css" rel="stylesheet">
...
<!-- /页头 -->
	...
	<div class="container" >
        <div class="row" style="margin-top:20px;">
			<?php if(is_array($tags)): $i = 0; $__LIST__ = $tags;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Article/tag?id='.$vo['id']);?>"><span class="label label-md label-info"><?php echo ($vo["tag_name"]); ?></span> </a><?php endforeach; endif; else: echo "" ;endif; ?>
		</div>
        <div class="row">
			<div class="col-md-12 article-list">
				<ul>
					<?php $__CATE__ = D('Category')->getChildrenId($category['id']);$__LIST__ = D('Document')->page(!empty($_GET["p"])?$_GET["p"]:1,10)->lists($__CATE__, '`level` DESC,`id` DESC', 1,true); if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li>
