    ...
    public function article_class_operat(){
        if(IS_AJAX)
        {
            ...
            else
            {
                ...
                {
                    M('article_class') -> where('ar_class='.I('get.id')) -> delete();
                    $save['ar_state'] = 3;
                    $save['ar_class'] = 1;
    ...
    public function article_publish(){
        ...
        else
        {
            if(I('get.id')){
                ...
                $article['ar_body'] = M('article_data') -> where('ar_id='.I('get.id')) -> getField('ar_body');
                $this -> assign('article',$article);
            }
            // 获取类别信息
            $article_class = M('article_class') -> select();
    ...
    public function article_operat(){
        if(IS_POST){
            ...
            if(I('get.id')){
                ...
                M('article_data') -> where('ar_id='.I('get.id')) -> save($saveData);
