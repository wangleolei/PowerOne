...
function change_comment($data,$place){
    for($i=0; $i < count($data); $i++){
        switch ($data[$i]['co_user_type']){
            ...
            default:
            {
                $user = M('user_admin') -> where('ad_id='.$data[$i]['co_user']) -> find();
...
function change_mood($data,$place){
    for($i=0; $i < count($data); $i++) {
        ...
        $user = M('user_admin') -> where('ad_id='.$data[$i]['no_user']) -> find();
...
function get_class_title($id){
    return M('article_class') -> where('ar_class='.$id) -> getField('ar_c_title');
}
