...
<head lang="en">
    ...
    <title>资讯文档管理 - Toilove Admin管理系统</title>
    <!-- ico/css/js -->
...
<article id="main-right">
    ...
    <hr />
    <!-- 编辑器 -->
    <script type="text/javascript" charset="utf-8" src="/toilove.com/Public/ueditor/ueditor.config.js"></script>
    <script type="text/javascript" charset="utf-8" src="/toilove.com/Public/ueditor/ueditor.all.min.js"></script>
    <form class="article-publish" role="form" action="<?php echo U('article_operat?id='.$article['ar_id']);?>" method="post" enctype="multipart/form-data">
            ...
            <div class="col-xs-4">
                <div class="input-group">
                    ...
                    <select class="form-control text-center" name="ar_class">
                        <?php if(is_array($article_class)): $i = 0; $__LIST__ = $article_class;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$classData): $mod = ($i % 2 );++$i;?><option value="<?php echo ($classData['ar_class']); ?>" <?php if(($classData['ar_class']) == $article['ar_class']): ?>selected<?php endif; ?> ><?php echo ($classData['ar_c_title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            <script id="editor" type="text/plain" style="height:400px;" name="ar_body" ><?php echo ($article['ar_body']); ?></script>
