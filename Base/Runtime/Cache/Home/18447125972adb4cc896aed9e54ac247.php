...
<head>
    ...
    <!-- seo标签、基础参数 -->
    <title><?php echo ($search); ?> - 搜索结果<?php if($site['site_name_suffix']): ?>- <?php echo ($site['site_name_suffix']); endif; ?></title>
    ...
    <!-- ico/css/js -->
    <link rel="shortcut icon" href="<?php echo ($site['site_url']); echo ($site['site_catalog']); ?>/Public/img/favicon.ico" />
    <link rel="stylesheet" href="//cdn.bootcss.com/bootstrap/3.3.5/css/bootstrap.min.css">
    <link href="<?php echo ($site['site_url']); echo ($site['site_catalog']); ?>/Public/css/system.home.css" rel="stylesheet">
</head>
<body>
<!--[if lte IE 8]>
...
<nav class="main-nav">
    <section class="container">
        <a href="<?php echo U('/');?>" title="博客首页"><img src="<?php echo ($site['site_url']); echo ($site['site_catalog']); echo ($site['site_logo']); ?>" alt="LOGO图" /></a>
...
<section class="main-content container">
    ...
    <ul class="module-page-1 pull-right">
        <?php if($button['first'] != null): ?><li><a href="<?php echo U('/search/'.$search);?>">首页</a></li><?php endif; ?>
        <?php if($button['previous'] != null): ?><li><a href="<?php if($button['previous'] == 1): echo U('/search/'.$search); else: echo U('/search/'.$search.'_'.$button['previous']); endif; ?>">上一页</a></li><?php endif; ?>
        <?php if(is_array($button['num'])): $i = 0; $__LIST__ = $button['num'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$numData): $mod = ($i % 2 );++$i; if(($numData) == $button['on']): ?><li class="active"><a><?php echo ($numData); ?></a></li>
            <?php else: ?>
            <li><a href="<?php if($numData == 1): echo U('/search/'.$search); else: echo U('/search/'.$search.'_'.$numData); endif; ?>"><?php echo ($numData); ?></a></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
        <?php if($button['next'] != null): ?><li><a href="<?php echo U('/search/'.$search.'_'.$button['next']);?>">下一页</a></li><?php endif; ?>
        <?php if($button['end'] != null): ?><li><a href="<?php echo U('/search/'.$search.'_'.$button['end']);?>">末页</a></li><?php endif; ?>
    </ul>
    <div class="clear"></div>
</div>
</section>
